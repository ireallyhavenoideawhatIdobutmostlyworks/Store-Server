...
class CustomerControllerTest {
    ...
    void get_by_id_when_id_exist() throws Exception {
        // given
        CustomerEntity customerBeforeSave = TestDataCustomerEntity.Customer("some@test.email" + UUID.randomUUID());
        ...
        mvc
                ...
                .andExpect(jsonPath("$.username", Matchers.is(customerBeforeSave.getUsername())))
                .andExpect(jsonPath("$.email", Matchers.is(customerBeforeSave.getEmail())))
                .andExpect(jsonPath("$.isActive", Matchers.is(customerBeforeSave.isActive())))
                .andExpect(jsonPath("$.isCompany", Matchers.is(customerBeforeSave.isCompany())))
                .andExpect(jsonPath("$.postalCode", Matchers.is(customerBeforeSave.getPostalCode())))
                .andExpect(jsonPath("$.street", Matchers.is(customerBeforeSave.getStreet())))
                .andExpect(jsonPath("$.city", Matchers.is(customerBeforeSave.getCity())))
                .andReturn();
    }
    ...
    void get_customers_list_when_list_is_not_empty() throws Exception {
        ...
        MvcResult mvcResult = mvc
                ...
                .andDo(MockMvcResultHandlers.print())
                .andExpect(status().is(200))
                .andExpect(jsonPath("$[0].email", Matchers.is(existingCustomerEntityList.get(0).getEmail())))
                .andExpect(jsonPath("$[1].email", Matchers.is(existingCustomerEntityList.get(1).getEmail())))
                .andExpect(jsonPath("$[2].email", Matchers.is(existingCustomerEntityList.get(2).getEmail())))
                .andReturn();
    ...
    void edit_customer_when_data_are_correct() throws Exception {
        ...
        String emailExist = customerRepository.findAll().get(0).getEmail();
        CustomerPayload entityForPutRequest = TestDataCustomerPayload.Customer(emailExist);
    ...
    void not_edit_customer_when_id_email_not_exist() throws Exception {
        ...
        String emailNotExist = "email@not.exist" + UUID.randomUUID();

        CustomerPayload entityForPutRequestWithIncorrectData = TestDataCustomerPayload.Customer(emailNotExist);
    ...
    void not_edit_customer_when_id_and_email_not_belong_to_the_same_customer() throws Exception {
        ...
        CustomerPayload entityForPutRequestWithIncorrectData = TestDataCustomerPayload.Customer(emailBelongToSecondCustomer);
